...
func (d *MyDbMap) GetArtistsFromUserId(userId string) ([]ArtistInfo, error) {
	...
	cmd := "select Artist.artistId, Artist.name, Artist.url, Artist.iconUrl from Artist inner join ListenTo on Artist.artistId = ListenTo.artistId where LitenTo.listenCount >= 2 and ListenTo.userId = ?"
